...
class LearnerThread(threading.Thread):
    ...
    def __init__(self, local_evaluator, model_idx):
    ...
    def step(self):
        ...
        if replay is not None:
            with self.grad_timer:
                td_error = self.local_evaluator.compute_apply(replay, self.model_idx)[
                    "td_error"]
...
class EnsembleOptimizer(PolicyOptimizer):
    ...
    def _init(
        ...
        assert num_replay_buffer_shards == len(self.remote_evaluators)
        ...
        self.learners = [LearnerThread(self.local_evaluator, model_idx) for model_idx in range(num_replay_buffer_shards)]
        [learner.start() for learner in self.learners]
        ...
        with self.timers["sample_processing"]:
            for (ev, model_idx), sample_batch in self.sample_tasks.completed():
                ...
                if self.steps_since_update[ev] >= self.max_weight_sync_delay:
                    ...
                    if weights is None or any(learner.weights_updated for learner in self.learners):
        ...
        with self.timers["update_priorities"]:
            for learner in self.learners:
                while not learner.outqueue.empty():
                    ra, replay, td_error = learner.outqueue.get()
    ...
    def stats(self):
        ...
        timing["learner_grad_time_ms"] = round(
            1000 * self.learners[0].grad_timer.mean, 3)
        timing["learner_dequeue_time_ms"] = round(
            1000 * self.learners[0].queue_timer.mean, 3)
        stats = {
        ...
        debug_stats = {
            ...
            "learner_queue": self.learners[0].learner_queue_size.stats(),
        }
        if self.debug:
