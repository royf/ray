...
def _build_action_network(
    ...
    q_values = tf.reduce_mean(q_values, 0)
...
class ModelAndLoss(object):
    ...
    def __init__(
        ...
        ensemble_size = config["num_workers"]
...
class ASQGraph(object):
    def __init__(self, registry, env, config, logdir):
        ...
        with tf.variable_scope(q_outer_scope_name) as outer_scope:
            for model_idx in range(ensemble_size):
                ...
                with tf.variable_scope(q_inner_scope_name) as inner_scope:
                    ...
                    q_func_vars.append(_scope_vars(inner_scope.name))
            q_values = tf.stack(q_values)
            all_q_func_vars = _scope_vars(outer_scope.name)
        ...
        self.train_expr = [optimizer.apply_gradients(gv) for gv in self.grads_and_vars]

        # update_target_fn will be called periodically to copy Q network to
        # target Q network
        update_target_expr = []
        for var, var_target in zip(
            sorted(all_q_func_vars, key=lambda v: v.name),
    ...
    def compute_gradients(
            ...
            importance_weights, model_idx):
        td_err, grads = sess.run(
            [self.td_error[model_idx], self.grads[model_idx]],
            feed_dict={
                self.obs_t: obs_t,
                self.act_t: act_t,
                self.rew_t: rew_t,
    ...
    def apply_gradients(self, sess, grads, model_idx):
        assert len(grads) == len(self.grads_and_vars[model_idx])
        feed_dict = {ph: g for (g, ph) in zip(grads, self.grads[model_idx])}
        sess.run(self.train_expr[model_idx], feed_dict=feed_dict)

    def compute_apply(
            self, sess, obs_t, act_t, rew_t, obs_tp1, done_mask,
            importance_weights, model_idx):
        td_err, _ = sess.run(
            [self.td_error[model_idx], self.train_expr[model_idx]],
            feed_dict={
                self.obs_t: obs_t,
                self.act_t: act_t,
                self.rew_t: rew_t,
