...
class ASQEvaluator(PolicyEvaluator):
    ...
    def __init__(self, registry, env_creator, config, logdir, worker_index):
        ...
        self.asq_graph = models.ASQGraph(registry, env, config, logdir)
        ...
        self.asq_graph.update_target(self.sess)
    ...
    def sample(self):
        ...
        if self.config["worker_side_prioritization"]:
            td_errors = self.asq_graph.compute_td_error(
                self.sess, obs, batch["actions"], batch["rewards"],
                new_obs, batch["dones"], batch["weights"], self.model_idx)
            new_priorities = (
    ...
    def compute_gradients(self, samples, model_idx):
        td_err, grads = self.asq_graph.compute_gradients(
            self.sess, samples["obs"], samples["actions"], samples["rewards"],
            samples["new_obs"], samples["dones"], samples["weights"], model_idx)
        return grads, {"td_error": td_err}

    def apply_gradients(self, grads, model_idx):
        self.asq_graph.apply_gradients(self.sess, grads, model_idx)

    def compute_apply(self, samples, model_idx):
        td_error = self.asq_graph.compute_apply(
            self.sess, samples["obs"], samples["actions"], samples["rewards"],
            samples["new_obs"], samples["dones"], samples["weights"], model_idx)
